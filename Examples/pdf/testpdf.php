...
/* initiate pdf generator */
Utilities::initPDF();

/* it generate pdf and save with 2nd argument name in 3rd argument if the mode is 'D' then it will download the file on client pc and if
mode is 'F' then it will save file in the given location as 2nd argument */
/* 2nd argument can contain full path of pdf including name where it should be saved path can be absolute or relative but path must exists */
Utilities::generateBasicPDF($sHTML,'pdf/test.pdf','F');

/* it gives output to browser if 2nd and 3rd argument is not supplied */
//Utilities::generateBasicPDF($sHTML);
